...
class Yolo(nn.Module):
    ...
    def forward(self, input):
        ...
        output_2 = output_2.view(batch_size, torch.div(num_channel, 4, rounding_mode='trunc'), height, 2, width,
                                 2).contiguous()

        output_2 = output_2.permute(0, 3, 5, 1, 2, 4).contiguous()
        # swee
        # output_2 = output_2.view(batch_size, -1, int(height / 2), int(width / 2))
        output_2 = output_2.view(batch_size, -1, torch.div(height, 2, rounding_mode='trunc'),
                                 torch.div(width, 2, rounding_mode='trunc'))
